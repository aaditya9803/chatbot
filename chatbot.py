...
def get_chatbot_response(message):
    ...
    if "hello" in message or "hi" in message or "hey" in message or "today" in message:
        return f"Hi, Today we have <ul><li>{'</li><li>'.join(menu[days[today.weekday()]])}</li></ul>"
    elif "tomorrow" in message:
        if today.weekday() != 6:
            tomorrow = today.weekday() + 1
        else:
            tomorrow = 0
        return f"On {days[tomorrow]}s we have <ul><li>{'</li><li>'.join(menu[days[tomorrow]])}</li></ul>"
    elif "yesterday" in message:
        if today.weekday() != 0:
            yesterday = today.weekday() - 1
        else:
            yesterday = 6
        return f"On {days[yesterday]}s we have <ul><li>{'</li><li>'.join(menu[days[yesterday]])}</li></ul>"

    elif 'menu' in message.lower() and ('all' or 'whole') in message.lower():
        return f"Our menu includes: <ul><li>{'</li><li>'.join(list(prices.keys()))}</li></ul>"
    elif 'menu' in message.lower() or ('menu' and 'today') in message.lower():
        return f"Today we have <ul><li>{'</li><li>'.join(menu[days[today.weekday()]])}</li></ul>"
